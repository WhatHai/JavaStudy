...
**Exception（异常）:是程序本身可以捕获处理的异常**。

Exception 类有一个重要的子类 **RuntimeException**。RuntimeException 异常由Java虚拟机抛出。**NullPointerException**（要访问的变量没有引用任何对象时，抛出该异常）、**ArithmeticException**（算术运算异常，一个整数除以0时，抛出该异常）和 **ArrayIndexOutOfBoundsException** （下标越界异常）。

**注意：异常和错误的区别：异常能被程序本身处理，错误是无法处理。**

**运行时异常和受检异常**

- 运行时异常继承RuntimeException，通常编程判断处理，不需要编译期强制检查
- 受检异常在编译时强制检查，在方法中声明异常（classNotFoundException，IOException）

区别：

- ClassNotFoundException：运行过程中用类加载器加载class文件，如果没有在classpath中找到类，抛出这个异常，比如jdbc加载驱动
- 编译通过，运行时找不到class文件，抛出NoClassDefFoundError错误；或者Spring没有导入jackso的jar包，编译时可以通过，运行时抛出错误。

throw和throws区别：

- throw：在方法内手动抛出异常，一次抛出一个，如果方法体内不进行catch处理，则需要方法声明throws
- throws：用在方法声明时，声明方法内可能产生的异常，不处理直接向上层转
